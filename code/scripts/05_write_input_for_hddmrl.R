# Script name: 05_write_input_for_hddmrl.R
# Project: Neuroptimal
...
DESIGN <- "complete" # only times t0 and t2
...
if (DESIGN == "complete") {
  data_for_hddm <- tibble(
    ...
    block = prl_dat$block,
    q_init = 0.5,
    # split_by = as.integer(factor(as.character(paste0(prl_dat$time, prl_dat$time)))),
...
data_for_hddm_5$split_by <- ifelse(data_for_hddm_5$time == 0, 0, 1)
data_for_hddm_5$Time <- ifelse(
  data_for_hddm_5$time == 0, 't0', 't2'
)


head(data_for_hddm_5)
#                     subj_code is_neuroptimal        rt trial feedback response time
# 1: neurOptimRevLearn_low_s001              1 0.7494614     1        1        0    0
# 2: neurOptimRevLearn_low_s001              1 4.7723310     1        0        0    2
# 3: neurOptimRevLearn_low_s001              1 0.5687356     2        1        0    0
# 4: neurOptimRevLearn_low_s001              1 3.8188948     2        0        0    2
# 5: neurOptimRevLearn_low_s001              1 0.7035965     3        1        0    0
# 6: neurOptimRevLearn_low_s001              1 3.8740622     3        0        1    2
...
rio::export(
  data_for_hddm_5,
  here::here(
    "python", "data", "complete_design", "input_for_hddmrl_complete_design.csv"
  )
)
...
rio::export(
  lookup_table,
  here::here(
    "python", "data", "complete_design", "lookup_table.csv"
  )
)
