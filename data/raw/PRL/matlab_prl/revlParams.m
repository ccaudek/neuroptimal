function prep = revlParams(taskVersion, runTask, blue_img, orange_img)
...
% =========================================================================
volatility          = {'high', 'low'};
%taskVersion         = 2 - mod(block_of_trials, 2); % odd = high, even = low
blocksequences{1}   = [20 20 20 20 20 20 20 20]; % length needs to be even number! 
blocksequences{2}   = [40 40 40 40]; % length needs to be even number! 
blocks              = blocksequences{taskVersion};

prep.taskVersion    = taskVersion;
prep.volatility     = volatility{taskVersion};
prep.prob           = 90 / 100; % feedback contingency  MUST BE 0.7!!!!
prep.nStim          = 2;
prep.nt             = sum(blocks);
...
%     prep.cue{4} = fullfile(prep.dir.imgs, 'slot2_down.jpg'); % orange  
    ...
    prep.draw.fx        = 35; % radius of the fixation cross
    prep.stimDistance   = wdh / 1.25; % OR 175 distance of the stimuli from the centre
    prep.draw.rStim     = round(wdh / 2); % OR 85
    prep.draw.rSmile    = round(wdh / 5.5);% OR 45
    prep.draw.rFrame    = prep.draw.rStim + round(0.01*wdh); % 'radius' of the square outside of the pie
    prep.draw.wdFrame   = 15;  % frame width
